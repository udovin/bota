...
use crate::sim::{TERRAIN_CELLS, TERRAIN_RLE, rules};
...
impl Ground {
    ...
    pub fn of(map: &crate::sim::MapDef) -> Ground {
        match map.terrain {
            crate::sim::Terrain::Dota => Ground::decode(),
            crate::sim::Terrain::Flat => Ground::flat(),
        }
    }
    ...
    pub fn wire_rle(map: &crate::sim::MapDef) -> Vec<(u16, u8)> {
        match map.terrain {
            crate::sim::Terrain::Dota => TERRAIN_RLE.to_vec(),
            crate::sim::Terrain::Flat => {
                let mut out = Vec::new();
