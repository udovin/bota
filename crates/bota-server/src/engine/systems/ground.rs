...
use crate::engine::{Entity, TraceSide, World};
use crate::sim::{clamp_to_map, isqrt64, move_towards, rules};
...
impl World {
    ...
    fn step_free(&self, mover: Entity, from: Vec2, next: Vec2) -> bool {
        if !self.grid.walkable(next) && self.grid.walkable(from) {
            return false;
        }
        !self.blocked_by_bodies(mover, from, next)
    }
    ...
    pub fn march_step(&self, mover: Entity, aim: Vec2, step: Fixed) -> Vec2 {
        ...
        if self.step_free(mover, from, next) {
            next
        } else {
            from
        }
    ...
    pub fn march_aim(
        ...
        if self.step_free(mover, from, straight) {
            return (waypoint, None);
        }
        ...
        for turn in [Turn::Eighth, Turn::Quarter, Turn::ThreeEighths] {
            for side in [first, other_side(first)] {
                ...
                if ahead != from && self.step_free(mover, from, ahead) {
    ...
    pub fn walk_step(&self, mover: Entity, aim: Vec2, step: Fixed) -> Vec2 {
        ...
        if let Some(body) = self.blocking_body(mover, straight) {
            for side in [1, -1] {
                ...
                if slid != from && self.step_free(mover, from, slid) {
                    return slid;
                }
            }
        }
        ...
        for part in 1..=rules::STEP_FIT_TRIES {
            ...
            if closer != from && self.step_free(mover, from, closer) {
                return closer;
            }
        }
        from
