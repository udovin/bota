...
/// The plain form of a kind that has only one. A tower has one per tier and is
/// not among these.
pub fn unit_def(kind: UnitKind) -> Option<&'static UnitDef> {
    match kind {
        UnitKind::CreepMelee => Some(&MELEE_CREEP),
        UnitKind::CreepFlagbearer => Some(&FLAGBEARER_CREEP),
        UnitKind::CreepRanged => Some(&RANGED_CREEP),
        UnitKind::CreepSiege => Some(&SIEGE_CREEP),
        UnitKind::Hero => Some(&HERO),
        UnitKind::Roshan => Some(&ROSHAN),
        UnitKind::Ancient => Some(&ANCIENT),
        UnitKind::Fountain => Some(&FOUNTAIN),
        _ => None,
    }
}
